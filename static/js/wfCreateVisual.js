...
// Fill in the dropdown box
const dropdown = document.getElementById("countryCodeGraph2");

// Load JSON data using d3.json
d3.json(urlGDP).then(data => {
const addedCountryCodes = new Set(); // Set to keep track of added country codes
addedCountryCodes.add("United States");// Set default

// Populate the dropdown with options
   data.forEach(country => {
      if (!addedCountryCodes.has(country.Country_Code)) {
        const option = document.createElement("option");
        option.value = country.Country_Code;
        option.textContent = country.Country.replace(/_/g, ' '); // Replace underscores with spaces
        dropdown.appendChild(option);
        addedCountryCodes.add(country.Country_Code); // Add to set to avoid duplicates
      }
    });
  });
...
function getCountryCode() {
  ...
  d3.json(urlGDP).then (function(GDPdata) { 
    ...
    // Fixed the typo
    d3.select("countryCodeGraph2").dispatch("change");
